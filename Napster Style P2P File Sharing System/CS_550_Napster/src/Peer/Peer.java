...
public class Peer {
	 ...
	 */
	public static void fileMonitor(String path, peerFunction peerfunction){
		File file = new File(path);
		String test[];
		test = file.list();
		if(test.length!=0){
			for(int i = 0; i<test.length; i++){
//				registerThread(test[i],peerfunction);
			}
		}
		// Use WThread to auto update register file
		new WThread(path,peerfunction);
	}
	 ...
	 */ 
	public static void registerThread(String fileName, peerFunction peerfunction){
		Socket socket = null;
	 ...
	 */ 
	public static void unregisterThread(String fileName, peerFunction peerfunction){
		Socket socket = null;
		...
		try{			
			...
			peerfunction.unregister(peerInfo.local.ID, fileName);
	...
	/*  Search function
	 *  Set up a socket connection to the index server
	 *  Search the file in the index server
	 */
	public boolean searchThread(String fileName, peerFunction peerfunction){
		Socket socket = null;
		...
		try{			
			...
			while(!("bye".equals(findID = br.readLine()))){
				peerInfo.dest.destList.add(findID);							
			}
			
			// If find file in some peers, output their address
			if((find = peerfunction.search(fileName))== true){
				for(int i=0; i<peerInfo.dest.destPath.size(); i++){
					System.out.println(fileName+" is found on "+peerInfo.dest.destPath.get(i));
				}
			}
	 ...
	 */
	public void download(String fileName, peerFunction peerfunction){
		// Get IP and Port of the peer which has this file
		// Resolve address
		String IP = null;
		String folder = null;
		int port = 0;
		...
		String address = peerInfo.dest.destPath.get(0);
		 ...
		 *  Set up serverPort
		 */
		serverPort = peerInfo.local.downloadPort;
	...
	public void talk(peerFunction peerfunction)throws IOException{
		...
		while(!exit){
			...
			case 1:{					
				...
				registerThread(fileName,peerfunction);			
				break;					
			}
	...
	public static void main(String args[])throws IOException{
	    peerFunction peerfunction = new peerFunction();
	    peerfunction.intialize();
	    fileMonitor(peerInfo.local.path,peerfunction);
	    ServerSocket server = null;
	    try{
	    	server = new ServerSocket(peerInfo.local.serverPort);
...
class WThread extends Thread {
	String path = null;
	peerFunction peerfunction = null;
//	peerInfo.local.fileList
	public WThread(String path,peerFunction peerfunction){
		this.path = path;
				...
				if(peerInfo.local.fileList.size()!=0){
					for(int i = 0; i < peerInfo.local.fileList.size(); i++){
						...
						if(!file.exists()){
							...
							Peer.unregisterThread(peerInfo.local.fileList.get(i),peerfunction);
...
class peerSocket{
	...
	private peerFunction pf = new peerFunction();
	
	public peerSocket()throws IOException{
		pf.intialize();
		socket = new Socket(peerInfo.local.IP,peerInfo.local.clientPort);
	}
	
	public peerSocket(String IP, int port)throws IOException{
		pf.intialize();
		peerInfo.local.clientPort = port;
		socket = new Socket(IP,peerInfo.local.clientPort);
	}
...
class peerServer{
	...
	public peerServer()throws IOException{
		port = peerInfo.local.serverPort;
